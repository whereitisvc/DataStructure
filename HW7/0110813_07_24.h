...
typedef struct node
	{
	 ...
	 struct node* left;
	 struct node* right;
	 int isthread; // 0 for right-link is not a thread, 1 for right-link is a thread
	} NODE;
...
NODE* _insert (BST_TREE* tree, NODE* parent, NODE* newPtr)
{
    NODE* temp;
	...
	if (tree->compare(newPtr->dataPtr, parent->dataPtr) < 0)
    {
        temp = _insert(tree, parent->left, newPtr);
        if( newPtr->isthread == 0 )//if newPtr hasn't already become a thread, then create thread
        {
            newPtr->right = parent;
	        parent->left = temp;
	        newPtr->isthread = 1;
    ...
    {
        if( parent->isthread==0 ) temp = _insert(tree, parent->right, newPtr); //if parent is not a thread, keep going
        else temp = _insert(tree, NULL, newPtr); //if parent is a thread, then give NULL to stop going(base)
        if( newPtr->isthread == 0 )//if newPtr hasn't already become a thread, then create thread
        {
            newPtr->right = parent->right;
            parent->right = temp;
            newPtr->isthread = 1;
            parent->isthread = 0;//parent right-link is not a thread now, so set to 0
        }
        return parent;
...
{
     NODE* pos = tree->root;
     while( pos->left != NULL ) pos = pos->left; // start with leftmost node
     
     while( pos != NULL )
     {
            process(pos->dataPtr);
            if( pos->isthread == 0 ) //if pos is not a thread
            {
                pos = pos->right;
                while( pos->left != NULL ) pos = pos->left; //start with leftmost node
            }
